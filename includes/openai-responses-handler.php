...
class SV_OpenAI_Responses_Handler
{
    ...
    public function call_responses_api($model, $input, $options = [])
    {
        try {
            ...
            // } 
            // temperature is not supported with gpt5

            if (!empty($options['max_output_tokens'])) {
                $body_data['max_output_tokens'] = intval($options['max_output_tokens']);
            }

            if (!empty($options['instructions'])) {
                $body_data['instructions'] = $options['instructions'];
            }
            ...
            if (!empty($options['response_format'])) {
                ...
                } elseif ($response_format['type'] === 'json_schema' && !empty($options['response_schema'])) {
                    $body_data['text'] = [
                        'format' => [
                            'type' => 'json_schema',
                            'name' => $options['schema_name'] ?? 'default_schema',
                            'strict' => true,
                            'schema' => $options['response_schema']  // Use the separate schema object
                        ]
                    ];
                } else {
                    $body_data['text'] = [
    ...
    // Fallback for older response structures or different formats
