...
function renderForm() {
	const quizDiv = document
		.getElementById("planner-personality-type-quiz")
		.getElementsByClassName("qa-container")[0];
...
function QuizRender() {
	...
	const tieBreakerQuestions = [
		{
			question:
				"Kai turi išspręsti svarbų klausimą, kaip renkiesi ieškoti sprendimo?",
			answers: [
				{
		...
		{
			question:
				"Kaip vertini nenumatytus pasikeitimus (projektuose, užduotyse, planuose)?",
			answers: [
				{
	...
	const [showResultContainer, setShowResultContainer] = useState(
		cookieSet() > 0,
	);
	...
	const [questionsAnswered, setQuestionsAnswered] = useState({
		...getQuestionSet(currentQuestion),
	});
	...
	const handleChange = (answerLetter, points, variantIndex) => {
		...
		setQuestionsAnswered((prevQuestionsAnswered) => {
			...
			setCurrentVariant(
				Object.values(updatedQuestionsAnswered).findIndex(
					(answer) => answer === 0,
				),
			);
			...
			if (currentQuestion === quizData.length - 1) {
				pairs.forEach(({ key1, key2, index }) => {
					if (updatedAnswers[key1][0] === updatedAnswers[key2][0]) {
						...
						setDichotomy((prevDichotomy) => {
							const updatedDichotomy = [...prevDichotomy];
							updatedDichotomy[index] =
								updatedAnswers[key1][0] > updatedAnswers[key2][0] ? key1 : key2;
	...
	useEffect(() => {
		...
		if (
			Object.values(dichotomy).find((letter) => letter === "") === undefined
		) {
			const typeString = dichotomy.reduce(
			...
			const memePath = imgFolder(
				`./type-meme/${typeString.toLowerCase()}_meme.webp`,
			);
			const memeUrl =
				typeof memePath === "object" ? memePath.default : memePath; // Ensure it's a string URL
			setTypeImg(imgUrl);
			setTypeMeme(memeUrl);
			setType(typeString);
			setTieBreak([]);
	...
	const showResult = () => {
		...
		document.cookie = `productivityType=${dichotomy.reduce(
			(prev, current) => prev + current,
			"",
	...
	const sendEmail = () => {
		...
		if (
			!validateEmail(subscriberData.email) ||
			!validateName(subscriberData.name)
		) {
		...
		<>
			{quizData[currentQuestion] &&
				quizData[currentQuestion].answers.map((answer, answerIndex) => (
					<div
						className={`variant ${
							currentVariant === answerIndex ? "active" : "inactive"
						}`}
						id={`variant-${answer.letter}`}
						key={`variant-${answer.letter}`}
					>
						<p
							key={`q-${answer.letter}`}
								...
								return (
									<>
										<input
											...
											value={points}
											onChange={() =>
												handleChange(answer.letter, points, answerIndex)
											}
			...
			{tieBreak.length > 0 &&
				tieBreak.map((q, index) => {
					return (
						<div id="tie-break" key={index}>
							<div
								className={`variant ${
									currentTieBreakQuestion !== index ? "inactive" : "active"
								}`}
							>
				...
				<img
					style={{
						display:
							!tieBreak.length > 0 &&
							!quizData[currentQuestion] &&
							Object.values(dichotomy).find((letter) => letter === "") ===
								undefined &&
							!showResultContainer
								? "block"
								: "none",
				...
				<>
					<div className="result-container">
						{!type || !quizAnswer[type] ? (
							<div>
								...
								<p>
									Jei puslapis automatiškai neatsinaujino, atnaujinkite rankiniu
									būdu
								</p>
							</div>
						) : (
					...
					{!emailSent && (
						<>
							<div className="contact-form">
								<div>
									<strong>OHO! </strong>Apie tavo produktyvumo tipą dar tieeeeek
									galima papasakoti, kad net į puslapį netelpa 😅 Įsivaizduoji?!
									<br />
									<br />
								...
								/>
								<span className={`error ${errors.name ? "" : "hidden"}`}>
									nurodyk savo vardą
								</span>
								<input
									type="email"
								...
								<div className="checkbox-container">
									...
									<label htmlFor="subscribe">
										sutinku prenumeruoti 12GM naujienlaiškį
									</label>
								</div>
							</div>
							<button
							...
							>
								{sending
									? "Siunčiama..."
									: "Noriu visos tiesos apie savo produktyvumo tipą!"}
					...
					{emailSent && !emailError && (
						<div className="email-sent">
							Puiku! Pasitikrink el. paštą (jei nerandi laiško, peržiūrėk ir
							spam aplanką)
						</div>
					)}
					{emailError && (
				...
				<>
					...
					<button
						...
						className={`next-question-button ${
							Object.values(questionsAnswered).find(
								(answer) => answer === 0,
							) === undefined
								? "variant active"
								: ""
